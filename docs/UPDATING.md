...
- [ ] Version: bump `NAME` in `app/src/main/java/com/bmscompanion/app/AppVersion.kt` (and `BMS` for a new BMS release) plus `versionCode` in `app/build.gradle.kts`. The APK's `versionName`, the PC installer's version and the About page all read that one file.
...
- [ ] Install the MSI on a machine that already has the previous version: the running app is closed for you, the old version disappears from Installed apps (one entry, not two), and `%APPDATA%\BMS Companion` is recreated empty. `finish-msi.ps1` prints the three sequence positions and fails the build if they are wrong.
